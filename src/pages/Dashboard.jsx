    ...
    <div style={{ padding: "24px 40px", background: "#f0f4fa", minHeight: "100vh", overflowY: "auto", width: "100%" }}>
      ...
      <div style={{ marginBottom: "32px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "20px", marginBottom: "20px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
            <button onClick={() => navigate("/ticketmanager/2")} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "12px 28px", background: "linear-gradient(135deg, #0B3D91, #1a52b8)", border: "none", borderRadius: "40px", color: "#fff", cursor: "pointer", fontWeight: "600", fontSize: "14px" }}><ArrowLeft size={18} /><span>Back to Main Page</span></button>
            <div style={{ width: "2px", height: "35px", background: "#e2e8f0" }} />
            <div><div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
              <div style={{ width: "56px", height: "56px", background: "linear-gradient(135deg, #0B3D91, #1a52b8)", borderRadius: "18px", display: "flex", alignItems: "center", justifyContent: "center" }}><LayoutDashboard size={30} color="#fff" /></div>
              <div><h1 style={{ fontSize: "30px", fontWeight: "700", color: "#0a2540", margin: 0 }}>{greeting}, Astghik!</h1><p style={{ color: "#64748b", marginTop: "6px", fontSize: "14px" }}>{formatDate()} • {stats.total} total tickets</p></div>
            </div></div>
          </div>
        ...
        <div style={{ background: "linear-gradient(135deg, #0B3D91, #1a52b8)", borderRadius: "20px", padding: "24px 32px", marginTop: "20px", color: "#fff", position: "relative", overflow: "hidden" }}>
          <div style={{ position: "absolute", right: "-20px", top: "-20px", opacity: 0.1 }}><Activity size={140} /></div>
          <div><h2 style={{ fontSize: "22px", fontWeight: "700", marginBottom: "10px" }}>🚀 Welcome to Your Support Dashboard</h2><p>{stats.resolved} resolved this month • {stats.slaCompliance}% SLA • {stats.monthlyGrowth} growth</p></div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(8, 1fr)", gap: "16px", marginBottom: "32px" }}>
        {[
          ...
          { icon: <Target size={20} />, label: "SLA", value: stats.slaCompliance + "%", change: "+2%", color: "#8b5cf6", bg: "rgba(139,92,246,0.1)" },
          { icon: <Heart size={20} />, label: "Satisfaction", value: stats.satisfaction, change: "+5%", color: "#ec489a", bg: "rgba(236,72,153,0.1)" },
      ...
      <div style={{ background: "#fff", borderRadius: "20px", border: "1px solid #e9eef3", overflow: "hidden", marginBottom: "32px" }}>
        <div style={{ padding: "20px 24px", borderBottom: "1px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <button onClick={() => navigate("/tickets")} style={{ padding: "8px 20px", background: "#f1f5f9", border: "none", borderRadius: "30px", cursor: "pointer" }}>View All <ChevronRight size={16} /></button>
        </div>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>{recentTickets.map((t, i) => { const sc = getStatusColor(t.status); return (<tr key={t.id} style={{ borderBottom: "1px solid #f1f5f9" }}><td style={{ padding: "14px", fontFamily: "monospace", color: "#3b82f6" }}>{t.id}</td><td>{t.title}</td><td><div style={{ display: "flex", alignItems: "center", gap: "6px" }}><div style={{ width: "8px", height: "8px", background: sc.dot, borderRadius: "50%" }} /><span style={{ background: sc.bg, color: sc.color, padding: "2px 10px", borderRadius: "20px", fontSize: "11px" }}>{t.status}</span></div></td><td>{getPriorityBadge(t.priority)}</td><td><div style={{ display: "flex", alignItems: "center", gap: "6px" }}><div style={{ width: "28px", height: "28px", background: "linear-gradient(135deg, #e0e7ff, #c7d2fe)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "11px" }}>{t.assignee.slice(0,2)}</div>{t.assignee}</div></td><td style={{ color: "#64748b" }}>{t.department}</td><td style={{ fontWeight: "500", color: "#10b981" }}>{t.responseTime}</td><td style={{ color: "#64748b" }}>{t.date}</td><td><button onClick={() => navigate(`/ticket/${t.id}`)} style={{ padding: "6px 14px", background: "#eef2ff", border: "none", borderRadius: "8px", cursor: "pointer", fontSize: "12px", color: "#0B3D91" }}>View</button></td></tr>) })}</tbody>
          </table>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px" }}>
        ...
        <div style={{ background: "linear-gradient(135deg, #0B3D91, #1a52b8)", borderRadius: "20px", padding: "28px", color: "#fff" }}>
          ...
          <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", marginBottom: "28px" }}>
            <button onClick={() => navigate("/create-ticket")} style={{ padding: "12px 22px", background: "rgba(255,255,255,0.2)", borderRadius: "40px", cursor: "pointer", color: "#fff" }}>+ New Ticket</button>
            <button onClick={() => navigate("/tickets")} style={{ padding: "12px 22px", background: "rgba(255,255,255,0.2)", borderRadius: "40px", cursor: "pointer", color: "#fff" }}>View All</button>
            <button onClick={() => navigate("/reports")} style={{ padding: "12px 22px", background: "rgba(255,255,255,0.2)", borderRadius: "40px", cursor: "pointer", color: "#fff" }}>Reports</button>
          </div>
